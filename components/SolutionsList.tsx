...
export default function SolutionsList() {
  ...
  return (
    <section ref={containerRef} className="py-32 bg-slate-950/90 relative overflow-visible">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full grid grid-cols-1 lg:grid-cols-12 gap-16 items-start relative z-10">
        ...
        <div className="lg:col-span-5 space-y-8 lg:sticky lg:top-36">
          ...
          <div className="pt-4">
            <Link
              href="/contact"
              className="inline-flex items-center gap-2 px-6 py-3 border border-slate-800 hover:border-blue-500 text-slate-200 hover:text-white rounded-full bg-slate-900/50 hover:bg-slate-900 transition-all font-semibold cursor-pointer"
            >
              Request a proposal
        ...
        {/* Right Side Vertical Stack of Cards */}
          ...
          {solutionItems.map((item, index) => {
            ...
            return (
              <Link 
                key={index}
                href={item.href}
                className={`group block p-8 rounded-2xl transition-all duration-500 border ${
                  isPassed 
                    ? "bg-slate-900 border-blue-500/30 scale-[1.01] shadow-2xl" 
              ...
              >
                <div className="flex items-center justify-between gap-6">
                  <div className="flex items-center gap-6">
                    {/* Number block */}
                    <span className={`text-sm font-bold font-mono w-10 h-10 rounded-xl flex items-center justify-center border transition-all ${
                      isPassed 
